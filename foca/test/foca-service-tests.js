...
describe('Foca Service Test', () => {
    ...
    it('should get the list of available leagues', done => {
        focaService.getLeagues()
        .then(runAsserts)
        .catch(() => {
            assert.fail('Getting leagues list failed.')
            done()
        })

        function runAsserts(body){
    ...
    it('should get a specific league specified by its id', done => {
        focaService.getLeaguesById(2001)
        .then(runAsserts)
        .catch(() => {
            assert.fail('Getting league by id failed.')
            done()
        })
    ...
    it('should get the list of groups', done => {
        focaService.getGroupList()
        .then(runAsserts)
        .catch(() => {
            assert.fail('Getting groups list failed.')
            done()
        })
    ...
    it('should add a group of favorite teams', done => {
        focaService.postGroup('New Group', 'New group description')
        .then(runAsserts)
        .catch(() => {
            assert.fail('Adding group to list failed.')
            done()
        })
    ...
    it('should edit a group of favorite teams specified by its id', done => {
        focaService.editGroup('New Group Name', 'New group description', 'OzaIJmcBiLhWrTB2aZGt')
        .then(runAsserts)
        .catch(() => {
            assert.fail('Editing group information failed.')
            done()
        })
    ...
    it('should get the detailed information from a group given its id', done => {
        focaService.getGroupById('i8kJJGcBSAafA5skJ5RO')
        .then(runAsserts)
        .catch(() => {
            assert.fail('Getting group by id failed.')
            done()
        })
    ...
    it('should add a team to a group of favorite teams given both ids', done => {
        focaService.addTeamToGroup('i8kJJGcBSAafA5skJ5RO', 30)
        .then(runAsserts)
        .catch(() => {
            assert.fail('Adding team to group failed.')
            done()
        })
        ...
        function runAsserts(){
            ...
            assert.equal(group._source.teams[3], 30)
    ...
    it('should remove a team from a group of favorite teams given both ids', done => {
        focaService.removeTeamFromGroup('i8kJJGcBSAafA5skJ5RO', 52)
        .then(runAsserts)
        .catch(() => {
            assert.fail('Deleting team from group failed.')
            done()
        })
    ...
    it('should get the matches from every team of the group given its id', done => {
        focaService.getMatchesByGroup('qqwTnmcBNosGXavWDAAN', undefined)
        .then(runAsserts)
        .catch(() => {
            assert.fail('Getting matches from group teams failed.')
            done()
        })
